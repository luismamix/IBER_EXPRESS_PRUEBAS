...
function init() {
  ...
  $('#embalaje_caja').click(function (e) {
    ...
    let mogollon = "<div class='form-check form-check-inline'>"
      + "<input class='form-check-input' type='radio' id='caja_grande' name='cod_embalaje' value='CJ01' required>"
      + "<label class='form-check-label supersizeme' for='caja_grande'>Grande Max 30 kg</label>"
      + "</div>"
  ...
  $('#embalaje_caja_botellas').click(function (e) {
    ...
    //crear los radio button para elegir el tamaño
    let mogollon = "<div class='form-check form-check-inline'>"
      + "<input class='form-check-input' type='radio' id='caja_botellas_grande' name='cod_embalaje' value='CB01' required>"
      + "<label class='form-check-label supersizeme' for='caja_botellas_grande'>Grande Max 30 kg</label>"
      + "</div>"
      + "<div class='form-check form-check-inline'>"
...
function establecer_fecha_prevista_entrega() {
 ...
 let fhoy = new Date();
 let fprev_entrega = new Date();

 //limite inferior
 fprev_entrega.setDate(fhoy.getDate() + 1);
 fprev_entrega.setHours(fprev_entrega.getHours()+1);
 let dia = ("0" + fprev_entrega.getDate()).slice(-2);
 let mes = ("0" + (fprev_entrega.getMonth() + 1)).slice(-2);
 let año = fprev_entrega.getFullYear();
 let horas = ("0" + fprev_entrega.getHours()).slice(-2);
 let minutos = ("0" + fprev_entrega.getMinutes()).slice(-2);
 let segundos = ("0" + fprev_entrega.getSeconds()).slice(-2);
 let custom = año + "-" + mes + "-" + dia + "T" + horas + ":" + minutos + ":" + segundos;
 $('#fecha_prevista_entrega').val(custom);
}

function establecer_fecha_recogida() {
        ...
        if (response.length > 0) {
          for (item of response) {
            ...
            cadena += "<tr>"
              ...
              + "<td id=r_c" + i + "6><button id='a" + i + "' type='button' class='btn btn-lima cargar' >Cargar</button>"
              + "</tr>";
            i++;
          }
        ...
        if (response.length > 0) {
          for (item of response) {
            ...
            cadena += "<tr>"
              ...
              + "<td id=e_c" + i + "6><button id='a" + i + "' type='button' class='btn btn-lima cargar' >Cargar</button>"
              + "</tr>";
            i++;
          }
...
function cargarBuscador_oficinas_recogida() {
  let mogollon = "<h4><u>Buscador de Oficinas</u></h4>"
    ...
    + "<table id='r_tabla_resultado_busqueda' class='table table-lima rounded table-responsive'>"
    + "<thead>"
    + "<tr>"
    + "<th>Cod Oficina</th>"
    + "<th>Direccion</th>"
...
function cargarBuscador_oficinas_entrega() {
  let mogollon = "<h4><u>Buscador de Oficinas</u></h4>"
    ...
    + "<table id='e_tabla_resultado_busqueda' class='table table-lima rounded table-responsive'>"
    + "<thead>"
    + "<tr>"
    + "<th>Cod Oficina</th>"
    + "<th>Direccion</th>"
...
function esconderCajas() {
  $('#div_razon_social_remitente').hide();
  $('#div_ccc_remitente').hide();
  $('#div_modificardatosrte').hide();
  $('#div_modificardatosdest').hide();
  $('#div_r_dircompleta_of_citypaq').hide();
  $('#div_e_dircompleta_of_citypaq').hide();
  ...
  $('#div_nombre_remitente').show();
  $('#div_apellidos_remitente').show();
  $('#div_numtarjeta_remitente').show();
  $('#div_cod_tipo_envio').hide();
  $('#div_tipo_remitente').hide();
  $('#div_recogida_citypaq').hide();
  $('#div_entrega_citypaq').hide();
  $('#div_embalaje_caja').hide();
  $('#div_embalaje_caja_botellas').hide();
  $('#div_embalaje_rollo_tubo').hide();
  //mostrar (solo tipo envio valijas) / ocultar 
  $('#div_embalaje_valijas').hide();
  $('#razon_social_remitente').prop('required', false);
  $('#ccc_remitente').prop('required', false);
